...
//umbral:		Umbral para el calculo de confiabilidad
//simiter:		Numero de replicaciones a realizar por RVR
void NetworkDesign::NetworkDesignAlgorithm3(string input,int cantiter,int k,double umbral, int simiter){
	...
	for(int i=0; i < cantiter; i++){
		...
		//Prob Nodo 0.99 - Aristas 0.99
		ResultRVR r2 = r->ConfMethod((unsigned)time(0),simiter,g2);
		//Prob Nodo 0.99 - Aristas 0.97
		g2->SetEnabledEdgesProb(0.97);
		ResultRVR r3 = r->ConfMethod((unsigned)time(0),simiter,g2);
		//Prob Nodo 0.99 - Aristas 0.95
		...
		g2->SetEnabledEdgesProb(0.97);
		ResultRVR r9 = r->ConfMethod((unsigned)time(0),simiter,g2);
		//Prob Nodo 0.95 - Aristas 0.95
		g2->SetEnabledEdgesProb(0.95);
		ResultRVR r10 = r->ConfMethod((unsigned)time(0),simiter,g2);
		...
		//TODO: Agregar los resultados de las confiabilidades que faltan
		//Agrego los datos al log de la iteracion
		FILE * f3 = fopen(path2.data(), "a");
		char * iter = new char[4];
		sprintf(iter,"%i",i);
		fprintf(f3,"%s\t%f\t%f\t%f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%.10f\t%.15f\t%f\n",
				iter,costo1,costo2,costo3,r2.darMedia(),r2.darVar(),r3.darMedia(),r3.darVar(),
				r4.darMedia(),r4.darVar(),r5.darMedia(),r5.darVar(),r6.darMedia(),r6.darVar(),
				r7.darMedia(),r7.darVar(),r8.darMedia(),r8.darVar(),r9.darMedia(),r9.darVar(),
				r10.darMedia(),r10.darVar(),tiempo);
		fclose(f3);
		delete iter;

		if (r2.darMedia() >= umbral){
			g2->SetSteinerNodesProb(0.99);
			g2->SetEnabledEdgesProb(0.99);
			Result * r3 = new Result(g2->GetCost(),r2.darMedia(),g2);
			AddResult(results, r3);
		}
		...
		delete g2;
		delete r;
	} //Fin del For de las iteraciones
